...
import cn.kungreat.gateway.constant.GatewayConstant;
...
public class GlobalLoginOrRegisterFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (request.getURI().getPath().contains(GatewayConstant.LOGIN_URI)) {
            ...
            // header 中不能设置 null
            response.getHeaders().add("jwtToken",token);
            response.setStatusCode(HttpStatus.OK);
            return response.setComplete();
        }

        // 2. 如果是注册
        if (request.getURI().getPath().contains("temp-register")) {
        ...
        String token = headers.getFirst("jwtToken");
   ...
   /*     LoginUserInfo loginUserInfo = null;
    ...
    private String getTokenFromAuthorityCenter(ServerHttpRequest request, String uriFormat) {
        ...
        ServiceInstance serviceInstance = loadBalancerClient.choose("flybbs-base");
        ...
        Map requestBody = JSON.parseObject(
                parseBodyFromRequest(request), Map.class
        );
        LOGGER.info("login request url and body: [{}], [{}]", requestUrl,
