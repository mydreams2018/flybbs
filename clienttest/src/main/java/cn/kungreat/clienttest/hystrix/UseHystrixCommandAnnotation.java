...
public class UseHystrixCommandAnnotation {
    private final static Logger LOGGER = LoggerFactory.getLogger(UseHystrixCommandAnnotation.class);
    ...
    public List<ServiceInstance> getNacosClientInfo(String serviceId) {
        LOGGER.info("use hystrix command annotation to get nacos client info: [{}], [{}]",
                serviceId, Thread.currentThread().getName());
        return nacosClientService.getNacosClientInfo(serviceId);
    }

    /**
     * getNacosClientInfo 的兜底策略 - Hystrix 后备模式   入参返回一样
     * */
    public List<ServiceInstance> getNacosClientInfoFallback(String serviceId) {
        LOGGER.warn("can not get nacos client, trigger hystrix fallback: [{}], [{}]",
                serviceId, Thread.currentThread().getName());
